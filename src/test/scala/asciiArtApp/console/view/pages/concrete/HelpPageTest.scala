...
class HelpPageTest extends FunSuite{
  test("HelpPage render produces the expected content") {
    ...
    val actualText = HelpPage.render()

    assert(actualText == result, "HelpPage content did not match the expected value")
  }
}
